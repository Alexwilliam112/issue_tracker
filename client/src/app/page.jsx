...
const ISSUE_STAGES = ['Triage', 'Investigation', 'Implementation', 'Verification', 'Deployment', 'Monitoring'];
const ENVIRONMENTS = ['Development', 'Staging', 'Production', 'Disaster Recovery', 'N/A'];
const PROJECTS = ['Alpha API', 'Beta Frontend', 'Gamma DB', 'Delta Auth', 'Epsilon Mobile', 'Zeta Analytics', 'Omega Core'];
const ISSUE_TYPES = ['Bug', 'Incident', 'Change Request', 'Vulnerability', 'Feature Request', 'Tech Debt'];
const ROOT_CAUSE_CATS = ['Code Error', 'Configuration', 'Infrastructure', 'Third Party', 'Human Error', 'Capacity', 'Design Flaw', 'Data Quality'];
const RISK_OPTIONS = ['Data Loss', 'Security Breach', 'Compliance Violation', 'Financial Impact', 'Reputation Damage', 'SLA Breach'];
const MOCK_USERS = ['Alice Engineer', 'Bob Manager', 'Charlie Director', 'Diana VP', 'Evan CTO', 'Frank External', 'Grace Support', 'Heidi Ops', 'Ivan Security', 'Judy QA'];
...
const MultiSelectFilter = ({ options, selected, onChange, label, placeholder = "Filter..." }) => {
    ...
    useEffect(() => {
        ...
        function handleScroll() { if (isOpen) setIsOpen(false); }
        ...
        <div ref={wrapperRef} className="relative inline-block w-full md:w-auto">
            ...
            <button
                ...
                className={`
                    ...
                    ${selected.length > 0 ? 'border-indigo-300 text-indigo-600 bg-indigo-50' : 'border-slate-300 text-slate-600'}
                `}
            >
...
const SearchableSingleSelect = ({ value, onChange, options, label, placeholder = "Select...", icon: Icon, required }) => {
    const [isOpen, setIsOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState("");
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            {label && <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">{label} {required && <span className="text-red-500">*</span>}</label>}
            <div 
                onClick={toggleOpen}
                className={`
            ...
            {isOpen && createPortal(
                ...
                >
                    ...
                    <div className="max-h-48 overflow-y-auto p-1">
                        {filtered.length === 0 ? (
                            <div className="text-xs text-slate-400 text-center py-2">No results</div>
                        ) : (
                            filtered.map(opt => (
                                <button
                                    key={opt}
                                    ...
                                    className={`w-full text-left px-3 py-2 text-sm rounded flex items-center justify-between group
                                        ${value === opt ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-700 hover:bg-slate-50'}
                                    `}
                                >
                                    <span>{opt}</span>
                                    {value === opt && <CheckCircle2 className="w-3.5 h-3.5 text-indigo-500" />}
                                </button>
                            ))
                        )}
                    </div>
...
const UserSearchSelect = ({ availableUsers, onAdd }) => {
    ...
    const filtered = availableUsers.filter(u => u.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <>
            ...
            {isOpen && createPortal(
                ...
                >
                    ...
                    <div className="max-h-48 overflow-y-auto p-1">
                        {filtered.length === 0 ? (
                            <div className="text-xs text-slate-400 text-center py-2">No users found</div>
                        ) : (
                            filtered.map(user => (
                                <button
                                    key={user}
...
const RisksInput = ({ selectedRisks = [], onChange }) => {
    ...
    const available = RISK_OPTIONS.filter(r => 
      ...
      <div className="relative" ref={wrapperRef}>
        ...
        {isOpen && createPortal(
          ...
          >
            ...
            <div className="max-h-48 overflow-y-auto p-1">
                {available.length === 0 ? (
                    <div className="text-xs text-slate-400 text-center py-2">No remaining risks found</div>
                ) : (
                    available.map(risk => (
                    <button
                        key={risk}
...
export default function IssueTracker() {
  ...
  const filteredIssues = issues.filter(i => {
    ...
    const projectMatch = appliedFilters.project.length === 0 || appliedFilters.project.includes(i.project);
    const envMatch = appliedFilters.environment.length === 0 || appliedFilters.environment.includes(i.environment);
    const typeMatch = appliedFilters.issueType.length === 0 || appliedFilters.issueType.includes(i.issueType);
    ...
    if (appliedFilters.startDate) {
        dateMatch = dateMatch && new Date(i.reportedAt) >= new Date(appliedFilters.startDate);
    }
  ...
  const exportToCSV = () => {
    ...
    // Define headers
    const headers = [
      'ID', 'Title', 'Status', 'Stage', 'Project', 'Environment', 'Issue Type', 
      'Reported By', 'Reported At', 'Risk Score', 'Risks', 'Assignee', 
      'Context', 'Problem Statement', 'Evidence',
      'Root Cause Category', 'Root Cause Detail',
      'Latest Escalation Layer', 'Latest Escalation Status', 'Latest Escalation Remarks',
      'Agreed Resolution (Solution)', 'Agreed Resolution (Effort)',
      'Closed At'
    ];

    // Map data to rows
    const rows = filteredIssues.map(issue => {
        const latestEscalation = issue.escalations.length > 0 ? issue.escalations[issue.escalations.length - 1] : null;
        ...
        return [
            ...
            latestEscalation ? `"${(latestEscalation.remarks || '').replace(/"/g, '""')}"` : 'N/A',
            agreedResolution ? `"${agreedResolution.solution.replace(/"/g, '""')}"` : 'N/A',
  ...
  const validateIssue = (issue) => {
      const required = [
          'title', 'project', 'environment', 'issueType', 
          'reportedBy', 'reportedAt', 'context', 'problemStatement'
      ];
  ...
  const addEscalationLayer = () => {
    ...
    const newLayer = { id: Date.now(), layer: layerNum, stakeholders: [], status: 'Pending', remarks: '' };
  ...
  const updateEscalationRemarks = (layerId, newRemarks) => {
    setEditingIssue(prev => {
      if (!prev) return null;
      return { ...prev, escalations: prev.escalations.map(e => e.id === layerId ? { ...e, remarks: newRemarks } : e) };
    });
  };
      ...
      <div className="max-w-[98%] mx-auto mb-6">
         <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {STATUS_FLOW.map(status => {
                ...
                const isSelected = appliedFilters.status.includes(status) && appliedFilters.status.length === 1;
      ...
      <div className="max-w-[98%] mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-visible z-0 relative">
            ...
            {showFilters && (
                <div className="pt-2 animate-in fade-in slide-in-from-top-2 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <MultiSelectFilter label="Project" options={PROJECTS} selected={filterInputs.project} onChange={v => updateFilterInput('project', v)} />
                        <MultiSelectFilter label="Environment" options={ENVIRONMENTS} selected={filterInputs.environment} onChange={v => updateFilterInput('environment', v)} />
                        <MultiSelectFilter label="Issue Type" options={ISSUE_TYPES} selected={filterInputs.issueType} onChange={v => updateFilterInput('issueType', v)} />
                    ...
                    <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
                         <button onClick={resetFilters} className="px-6 py-2 rounded-lg text-sm text-slate-600 hover:bg-slate-100 font-bold border border-slate-200 transition-colors">Reset</button>
                         <button onClick={applyFilters} className="bg-indigo-600 text-white hover:bg-indigo-700 px-8 py-2 rounded-lg text-sm font-bold shadow-lg shadow-indigo-200 transition-all">Apply Filters</button>
                    </div>
                </div>
            )}
        </div>
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-slate-100">
                ...
                >
                    ...
                    <span className="text-sm font-normal text-slate-700 group-hover:text-indigo-600 transition-colors">{issue.title}</span>
                    </td>
                    <td className="px-6 py-4 text-xs text-slate-500 font-mono">
                    {new Date(issue.reportedAt).toLocaleDateString()}
                    </td>
                    <td className="px-6 py-4 text-xs text-slate-600">{issue.reportedBy}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{issue.project}</td>
                    <td className="px-6 py-4 text-xs text-slate-500">{issue.environment}</td>
                    <td className="px-6 py-4">
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold ${issue.riskScore > 30 ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>
                            {issue.riskScore}
                        </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs font-medium text-indigo-600">{issue.stage}</td>
                    <td className="px-6 py-4 text-xs text-slate-500">{issue.rootCauseCategory || '-'}</td>
                    <td className="px-6 py-4 text-xs text-slate-400">{issue.closedAt ? new Date(issue.closedAt).toLocaleDateString() : '-'}</td>
                </tr>
                ))}
                {paginatedIssues.length === 0 && (
                    <tr>
                        <td colSpan={12} className="px-6 py-8 text-center text-slate-400 text-sm italic">
          ...
          {paginatedIssues.map(issue => (
            <div key={issue.id} onClick={() => openViewModal(issue.id)} className="p-4 active:bg-slate-50 transition-colors cursor-pointer">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                 ...
                 {issue.stage && <span className="text-[10px] px-1.5 py-0.5 bg-indigo-50 text-indigo-700 rounded font-medium">{issue.stage}</span>}
              </div>
        ...
        <div className="px-4 md:px-6 py-3 border-t border-slate-100 bg-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-2 text-xs text-slate-500">
                ...
                >
                    {[100, 200, 300, 400, 500, 600, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000].map(val => (
                        <option key={val} value={val}>{val}</option>
                    ))}
                </select>
                <span className="ml-2">
                    {(pagination.page - 1) * pagination.limit + 1}-{Math.min(pagination.page * pagination.limit, filteredIssues.length)} of {filteredIssues.length}
                </span>
            </div>
        ...
        footer={
          <div className="flex flex-col-reverse md:flex-row justify-between items-center gap-4">
            <div className="flex w-full md:w-auto">
               {!isCreating && (
                 <button onClick={handleDelete} className="flex items-center gap-2 text-red-600 hover:text-red-700 hover:bg-red-50 px-4 py-2 rounded-lg transition-colors text-sm font-bold">
                ...
                <div className="max-w-6xl mx-auto space-y-6">
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <Card title="Issue Details" icon={Briefcase}>
                                        <div className="space-y-3">
                                            <SearchableSingleSelect required label="Project" options={PROJECTS} value={editingIssue.project} onChange={v => updateDraftField('project', v)} />
                                            <SearchableSingleSelect required label="Environment" options={ENVIRONMENTS} value={editingIssue.environment} onChange={v => updateDraftField('environment', v)} />
                                            <SearchableSingleSelect required label="Issue Type" options={ISSUE_TYPES} value={editingIssue.issueType} onChange={v => updateDraftField('issueType', v)} />
                                        </div>
                                    </Card>
                                    <Card title="Reporting" icon={User}>
                                        <div className="space-y-3">
                                            <div>
                                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Reported By <span className="text-red-500">*</span></label>
                                            ...
                                            <div>
                                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Reported At <span className="text-red-500">*</span></label>
                                            ...
                                            <SearchableSingleSelect label="Current Assignee" options={MOCK_USERS} value={editingIssue.assignee} onChange={v => updateDraftField('assignee', v)} />
                                        </div>
                                    </Card>
                                </div>
                                ...
                                <Card title="Description" icon={FileText}>
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Context <span className="text-red-500">*</span></label>
                                        ...
                                        <div className="p-3 bg-red-50 rounded-lg border border-red-100">
                                            <label className="block text-xs font-bold text-red-800 uppercase tracking-wide mb-1">Problem Statement <span className="text-red-500">*</span></label>
                                ...
                                <Card title="Analysis & Evidence" icon={Activity}>
                                     <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <SearchableSingleSelect label="Root Cause Category" options={ROOT_CAUSE_CATS} value={editingIssue.rootCauseCategory} onChange={v => updateDraftField('rootCauseCategory', v)} />
                                            <div>
                            ...
                            <div className="space-y-6">
                                <Card title="Status & Stage" icon={Activity}>
                                    <div className="space-y-4">
                                        ...
                                        <SearchableSingleSelect label="Issue Stage" options={ISSUE_STAGES} value={editingIssue.stage} onChange={v => updateStage(v)} />
                                    </div>
                                </Card>
                    ...
                    {activeTab === 'escalations' && (
                         ...
                         <div className="space-y-4">
                             ...
                             <div key={esc.id} className="bg-white border border-slate-200 rounded-xl p-4 md:p-6 shadow-sm relative overflow-hidden group/card">
                                 ...
                                 <div>
                                   <div className="flex items-center justify-between mb-3">
                                     ...
                                     <div className="relative group">
                                       <UserSearchSelect 
                                         availableUsers={MOCK_USERS.filter(u => !esc.stakeholders.some(s => s.name === u))}
                                   ...
                                   <div className="mt-6 pt-4 border-t border-slate-100">
                                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2 block">Result Remarks</label>
                                      <AutoTextArea 
                                        value={esc.remarks} 
                                        onChange={(e) => updateEscalationRemarks(esc.id, e.target.value)}
                    ...
                    {activeTab === 'resolutions' && (
                         ...
                         <Card title="Propose New Resolution" icon={Plus}>
                             ...
                             <div className="col-span-1 md:col-span-2">
                                 ...
                                 <textarea name="solution" required className="w-full border border-slate-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium resize-none" placeholder="Describe the fix..." rows={2} />
                             </div>
                             ...
                             <div>
                                 <label className="block text-xs font-bold text-slate-600 mb-1.5">Effort (Man Hours)</label>
                                 <div className="space-y-3">
                                 <input type="number" name="effort" min="0" required className="w-full border border-slate-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium" placeholder="e.g. 4" />
                         ...
                         <div className="grid grid-cols-1 gap-6">
                             ...
                             <div key={res.id} className={`border-2 rounded-xl overflow-hidden transition-all relative group/card ${res.isAgreed ? 'border-green-500 bg-green-50/20' : 'border-slate-200 bg-white'}`}>
                                 ...
                                 <div className="flex items-center gap-3 w-full pr-8">
                                     ...
                                     <div className="w-full">
                                        <AutoTextArea 
                                            value={res.solution} 
                                            onChange={(e) => updateResolution(res.id, 'solution', e.target.value)}
                                 ...
                                 <div className="space-y-1">
                                     ...
                                     <AutoTextArea 
                                        value={res.pros} 
                                        onChange={(e) => updateResolution(res.id, 'pros', e.target.value)}
                                 ...
                                 <div className="space-y-1">
                                     ...
                                     <AutoTextArea 
                                        value={res.cons} 
                                        onChange={(e) => updateResolution(res.id, 'cons', e.target.value)}
                                        className="text-slate-700 bg-transparent w-full text-sm"
                                        placeholder="Cons..."
                                     />
                                 </div>
                                 <div className="space-y-4">
                                     <div className="space-y-1">
                                         <h5 className="font-bold text-slate-500 uppercase text-[10px] tracking-wide">Effort (Hours)</h5>
                                         <input 
                                            type="number" 
                                            value={res.effort} 
                                            onChange={(e) => updateResolution(res.id, 'effort', parseInt(e.target.value))}
